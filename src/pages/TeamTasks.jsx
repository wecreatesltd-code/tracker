...
export default function TeamTasks() {
    ...
    const filteredTasks = tasks.filter(task => {
        const matchesSearch = task.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            (memberNames[task.assignedTo] || '').toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-12 sm:pb-24 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight transition-colors">Team Tasks</h2>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium transition-colors">Viewing all tasks across your contributing projects.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800 p-16 text-center transition-colors">
                    ...
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white transition-colors">No team tasks found</h3>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium transition-colors">Your contributing projects don't have any tasks matching your criteria.</p>
                </div>
            ) : viewMode === 'list' ? (
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 text-sm font-medium transition-colors">
                                        Assigned to: <span className="text-slate-700 dark:text-slate-300 font-bold">{memberNames[task.assignedTo] || 'Unassigned'}</span>
                                    </p>
                                </div>
                            </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 dark:bg-slate-800/50">
                                    ...
                                    <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">Assignee</th>
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2">
                                                <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-[10px] font-bold text-primary">
                                                    {(memberNames[task.assignedTo] || 'U').charAt(0)}
                                                </div>
                                                <span className="text-sm font-bold text-slate-600 dark:text-slate-400">{memberNames[task.assignedTo] || 'Unassigned'}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2">
